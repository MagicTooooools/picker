---
title: RAG 教程：从入门到掌握
url: https://itcharge.cn/uncategorized/rag-%e6%95%99%e7%a8%8b%ef%bc%9a%e4%bb%8e%e5%85%a5%e9%97%a8%e5%88%b0%e6%8e%8c%e6%8f%a1/
source: 程序员充电站
date: 2025-12-16
fetch_date: 2025-12-17T03:23:38.311640
---
...
[2025-12-17](https://itcharge.cn/uncategorized/rag-%E6%95%99%E7%A8%8B%EF%BC%9A%E4%BB%8E%E5%85%A5%E9%97%A8%E5%88%B0%E6%8E%8C%E6%8F%A1/)
...
```
flowchart TD
    A[「用户提问<br>（Query）」] --> B[「检索（Retrieval）<br>将问题转化为向量，在知识库中查找最相关的文档片段」]

    subgraph C [知识库构建（预处理）]
        direction LR
        C1[「原始文档<br>（PDF、Word、网页等）」] --> C2[「切分与向量化<br>（Chunking & Embedding）」] --> C3[「向量数据库<br>（Vector Database）」]
    end

    C3 --> B
    B --> D[「增强（Augmentation）<br>将检索到的文档片段与原始问题组合成新的提示」]
    D --> E[「生成（Generation）<br>LLM 基于增强后的提示生成最终答案」]
    E --> F[「输出答案<br>（附带引用来源）」]
```
...
* [未分类](https://itcharge.cn/category/uncategorized/)
